...
def get_radios():
    user_id = get_user_id()
    radios = Radio.query.filter_by(user_id=user_id).order_by(Radio.favorita.desc(), Radio.criado_em.desc()).all()
...
def get_radio(radio_id):
    user_id = get_user_id()
    radio = Radio.query.filter_by(id=radio_id, user_id=user_id).first()
    if not radio:
